...
| Dashboard, navigation, and notification brand | Payments-only MVP dashboard, actionable refresh errors, device toggle, crash-safe notification details, non-sticky icon badge, delayed lock test, and dollar icon available in internal TestFlight build 14 | `dashboard-navigation-and-brand.md` |
...
| Custom webhook payment sources | Production sender event received; always-visible activation and notification setup available in internal TestFlight build 14; first active live payment acceptance pending | `custom-webhook-payment-sources.md` |
