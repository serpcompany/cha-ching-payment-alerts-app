...
The top daily-summary card uses the saved reporting timezone and shows Gross volume, Payments, and Average payment in a compact Stripe-inspired summary: one label row and one value row at standard sizes, with a native Dynamic Type fallback. The selected day appears as the screen title rather than a third row inside the card. It opens on **Today**. Native toolbar buttons move backward one complete local calendar day at a time and forward only as far as Today. Historical days use their full timezone-local midnight-to-midnight window, including daylight-saving transitions, while Today ends at generation time. Counts may span currencies; monetary totals and averages never do. Horizontal swipe navigation is intentionally excluded from v1 until it has an on-device interaction design that does not compete with vertical scrolling or pull-to-refresh.
