...
The top daily-summary carousel uses the saved reporting timezone and shows Gross volume, Payments, and Avg. $ in a compact Stripe-inspired summary with exactly one label row and one value row at standard Dynamic Type sizes. The selected day appears as the large screen title rather than a row inside the card. It opens on **Today**. An iOS 17 native horizontal `ScrollView` uses view-aligned snapping, scroll-position identity, and content margins so adjacent days peek without a custom gesture competing with the vertical screen or pull-to-refresh. Swiping moves backward one complete local calendar day at a time and forward only as far as Today; nearby summaries are opportunistically prefetched and show a loading state without fabricated values. Historical days use their full timezone-local midnight-to-midnight window, including daylight-saving transitions, while Today ends at generation time. Counts may span currencies; monetary totals and averages never do.
