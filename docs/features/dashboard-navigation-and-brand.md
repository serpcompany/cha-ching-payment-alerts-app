...
The top daily-summary carousel uses the saved reporting timezone and shows Gross volume and Payments in two equal-width, centered columns with exactly one label row and one value row at standard Dynamic Type sizes. Every loading, short, zero, and long page shares one responsive width and height for the current content-size environment. Complete values are preserved; each column independently chooses the largest native text style that fits rather than ellipsizing, while accessibility sizes switch to a readable stacked metric layout and grow the shared card height. The selected day appears as the large screen title rather than a row inside the card. It opens on **Today**. An iOS 17 native horizontal `ScrollView` uses view-aligned snapping, scroll-position identity, and content margins so adjacent days peek without a custom gesture competing with the vertical screen or pull-to-refresh. The active scroll target IDs are stable semantic slots (older, selected, newer). On iOS 18 and later, native scroll-phase idle commits the selected slot and changes its represented date. Because iOS 17 does not expose scroll phase, its fallback locks the scroll as soon as the first semantic target is selected, debounces and commits while locked, recenters without animation, drains queued position updates, and reasserts the centered slot before accepting another gesture. Swiping therefore moves exactly one complete local calendar day per gesture and forward only as far as Today; nearby summaries are opportunistically prefetched and show a loading state without fabricated values. Historical days use their full timezone-local midnight-to-midnight window, including daylight-saving transitions, while Today ends at generation time. Counts may span currencies; monetary totals never do.

Reports default to **4 weeks**. A native menu offers 1 week, 4 weeks, 1 year, month to date, quarter to date, year to date, and all time. Gross volume and Payments include current and previous values, percentage-change state, and current/previous Swift Charts line series. Charts expose reporting-date axes and native legends; current data uses a solid accented line with point symbols while previous data uses a secondary dashed line, so comparison never depends on color alone. Semantic mark labels and concise summaries retain VoiceOver and Audio Graph exploration. Product and payment-source sections show both payment count and gross amount for the selected currency; neither value mixes currencies.
...
- Home's daily summary navigates backward through prior local days and forward only as far as Today.
