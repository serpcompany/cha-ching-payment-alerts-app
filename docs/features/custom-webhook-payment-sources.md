...
Migrations `0005` through `0009` and Worker version `5be363d1-2c16-4ff9-a644-a29c98791acd` were deployed on 2026-08-11 JST. The live `serp.store` source remains in setup with its existing source ID and private URL, but its old setup sample and mapping were removed. It now truthfully waits for the next real sender event; production still has zero custom-source payments and deliveries. The Worker returns `waiting`, `event_received`, `active`, or `paused` as an explicit connection state and does not set a synthetic unread badge on pushes. Migration `0009` adds the normalized notification-field snapshot used for custom pushes. The Worker also accepts an owner-authenticated delayed setup-event notification test, confirms an active registered phone, and places the exact preview body on Cloudflare Queue without creating a payment.

TestFlight build `13` (`65567b20-521c-49d8-a0e2-302d30a84fcd`) is `VALID` and `IN_BETA_TESTING` in the internal group **Cha-Ching Internal**. It contains the explicit **Waiting for first event** → **Event received** → **Active** flow, separate Notification settings, all-observed-fields list, direct field toggles, rename/remap/reorder editor, deterministic one-field-per-line body, genuine Apple notifications with crash-safe cold-launch full details, delayed lock-screen testing, non-sticky icon badges, cash-register sound, and focused Dashboard/navigation. Final custom-source acceptance requires the developer to send the next representative SERP Store event, opening the existing source on a signed iPhone, configuring and activating it, then sending one clearly identifiable live event and observing one matching payment and notification.
