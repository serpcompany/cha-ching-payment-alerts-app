...
- **Refresh connection health**: reloads the latest request evidence without claiming that the external sender was probed. While the request runs, the action identifies itself as refreshing. Completion stays visible beside the health evidence with a checked timestamp even when the classification is unchanged; failure appears in the same section and preserves the last known evidence.
...
- Refreshing connection health shows explicit in-progress, success, and failure feedback in the health section. An unchanged successful response still shows when it was checked, and a failed refresh does not erase the previously displayed evidence.
