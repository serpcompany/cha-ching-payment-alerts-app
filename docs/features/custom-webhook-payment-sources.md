...
8. Tap **Preview notification** to review the exact multiline body. **Test notification** sends that same observed event immediately as a genuine Apple notification without creating a payment. Apple's abbreviated preview shows only a few lines. Pressing a setup test opens its full standalone preview because no payment exists; pressing an active-source test selects Payments and opens the latest retained payment detail. **Test lock screen** asks the Worker to queue the exact observed event for a short delay and immediately shows inline guidance to lock the iPhone; no OK button delays the countdown. **Activate payment source** remains in the preview.

The sender owns the payload shape. Cha-Ching discovers every scalar value actually present in a valid sample payload up to 64 KiB and saves the user's mapping and notification design. A sample received during setup is never counted as revenue and never sends a notification. Changing a field toggle, label, mapping, or order invalidates the old preview, so activation always uses the design currently visible on screen. After activation, accepted payments also discover previously unseen scalar paths without retaining the raw payload.
...
7. **Active-source management** — reopening an activated source provides pause/resume, URL copy, developer-prompt copy, explicit URL regeneration, and notification presentation editing. The MVP applies renaming, hiding, reordering, and re-enabling to retained Payments details as well as future payments. Values enabled when a payment arrived remain archived by stable field ID when later hidden, so repeated off/on edits are reversible. A newly shown field appears on an older payment only when the value was retained when it arrived; Cha-Ching never invents or recovers discarded raw values. An unseen scalar path in an accepted payment is appended to Available fields with a readable path-derived label and its toggle on; its normalized value is retained and displayed for that payment. Preview, immediate test, and lock-screen test remain available after activation; tests use the latest saved payment values when available and safe example values otherwise. Active edits remain a local draft until the Worker accepts them; returning to management then shows **Notification settings saved.** Existing field IDs cannot be remapped to different payload paths after activation.

The Notification settings screen always pins **Activate payment source** above the bottom edge. It is visibly disabled with guidance until payment matching is complete and the current choices have been previewed. Previewing never hides activation inside the preview sheet: the user closes the preview, sees the enabled activation action in the same place, and explicitly puts the source live. Once active, new unique webhook events create payments and notifications.
...
- **Observed field** means a scalar path Cha-Ching has encountered in a setup sample or accepted active payment. It does not mean every theoretical field the sender could provide.
- **Available field** means an observed field offered for notification configuration. Availability does not imply that the field has a value in earlier payments.
- **Field catalog** remains reserved for a future sender-declared list that may include fields Cha-Ching has not observed.
- When an accepted active payment introduces an available field, Cha-Ching appends it to the saved order with a readable path-derived label and enables it immediately. The discovering payment includes its normalized value in Payments details and the ordinary payment notification. Earlier payments remain unchanged.
- A missing optional field stays configured with its existing toggle state and is omitted only from payments where it has no value. Cha-Ching does not alert, notify, start a capture mode, or automatically remove fields in response to discovery or absence.
...
- **Webhook activity**: source management passively shows the latest request evidence loaded when the screen opens. It has no manual check, refresh, reconnect, or repair control because Cha-Ching cannot probe the external sender. Rejected evidence explains that the sender payload must be fixed and resent. Quiet evidence says that silence may be normal when there are no payments and is not proof of disconnection.
...
- The scheduled health monitor sends one Apple notification per uninterrupted warning state. Pressing it selects Settings and pushes Payment sources. The affected source opens only after an owner-scoped load succeeds; transient failure retains a Retry route, and a confirmed missing source shows an unavailable message without rendering new-source setup. A later accepted event or duplicate clears the warning latch so a future outage can notify again.
...
- After activation, authenticated presentation-only edits preserve existing field IDs and payload paths. Automatic discovery creates a new stable field for an unseen scalar path; it never repoints an existing field. Renaming, hiding, reordering, and re-enabling update the presentation of existing payment details. A value that was enabled when the payment arrived remains in a private ID-keyed archive when later hidden, making repeated off/on edits reversible; a value never retained at arrival cannot be recovered. Future payments and notifications use the same accepted presentation.
...
- Enabled fields can appear on the iPhone lock screen. Newly discovered fields default on, so the sender's developer is responsible for keeping the payload sensible and the source owner may hide unwanted fields afterward. The designer warns the user to hide private fields before activation.
...
- Active-source management contains no manual webhook-health check. It presents the latest receiver-side evidence loaded with source detail and gives sender-side guidance for rejected or unexpectedly quiet activity without claiming connected or disconnected status.
...
- An accepted active payment automatically appends every previously unseen scalar path to Available fields with a readable label and its toggle on, and includes that normalized value in the discovering payment's details and ordinary notification.
- Automatic field discovery does not retain an active raw payload, rotate the private URL, alter earlier payments, emit a separate alert, require a capture mode, or remove a configured field merely because a later payment omits it.
...
Worker version `c52144ab-a5f2-44ba-bdfb-91913c75d2e0` was deployed on 2026-08-12 JST. Active-source test notifications carry the latest retained payment ID, so pressing one follows the normal Payments detail route. Setup tests remain standalone previews because no payment exists yet.
