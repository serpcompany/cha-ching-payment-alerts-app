...
- The scheduled health monitor sends one Apple notification per uninterrupted warning state. Pressing it selects Settings, pushes Payment sources, and opens the affected source. A later accepted event or duplicate clears the warning latch so a future outage can notify again.
