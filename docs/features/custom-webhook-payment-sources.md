...
7. Tap **Preview notification** to review the exact multiline body. **Test notification**, directly beneath Preview notification, sends that same sample-based notification to the user's registered iPhone without creating a payment. **Activate payment source** remains in the preview.
