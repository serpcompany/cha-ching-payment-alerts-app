...
7. Tap **Preview notification** to review the exact multiline body. **Test notification** sends that same sample immediately as a genuine Apple notification without creating a payment. Apple's abbreviated preview shows only a few lines; pressing it opens every selected line in Cha-Ching's full scrollable detail sheet. **Test lock screen** asks the Worker to queue the exact sample for a short delay and then tells the user to lock the iPhone. **Activate payment source** remains in the preview.
