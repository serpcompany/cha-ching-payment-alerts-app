...
- When an accepted active payment introduces an available field, Cha-Ching appends it to the saved order with a readable path-derived label and enables it immediately. The discovering payment includes its normalized value in Dashboard details and the ordinary payment notification. Earlier payments remain unchanged.
