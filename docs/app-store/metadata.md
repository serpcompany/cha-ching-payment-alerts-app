...
TestFlight build `5` (`5c34eebf-3352-4b1a-8c42-bcc97a043b95`) is valid and in beta testing with the internal group `Cha-Ching Internal` (`483e4a13-43c0-4658-9c1b-8238e7b9a773`). It includes production APNs, universal custom webhook sources, a copyable AI-agent/developer handoff, field mapping and notification preview, and Stripe/custom-source pause controls. Builds `1` through `4` are superseded.

The external group `Cha-Ching Beta` (`e95c8fca-3141-4ccb-917f-e7910bc37e0d`) contains `farleythecoder@gmail.com`, and build 5 is attached. The English What to Test notes and privacy URL are configured. Apple reports the build as `READY_FOR_BETA_SUBMISSION`; required beta-review contact details, including the phone number, still need to be supplied before external review can be submitted. Apple does not send the usable external invitation until that review is approved.
