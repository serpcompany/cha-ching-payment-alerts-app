...
TestFlight build `10` (`13dd4c3c-f5b5-4b69-a3f5-f7ed23c1dc3c`) is valid, `IN_BETA_TESTING`, and attached to the internal group `Cha-Ching Internal` (`483e4a13-43c0-4658-9c1b-8238e7b9a773`). It restores Apple's genuine foreground notification banner, keeps the complete selected payment details behind a notification press, removes the redundant Settings status row, and retains the real per-device notification switch and delayed lock-screen test. The signed archive and Apple's processed build-icon endpoint both expose the dollar-symbol `ChaChingDollarIcon`. Builds `1` through `9` are superseded for internal testing.

The external group `Cha-Ching Beta` (`e95c8fca-3141-4ccb-917f-e7910bc37e0d`) contains `farleythecoder@gmail.com`, and build 8 remains attached. Build 10 is internal-only until the missing beta-review contact details are supplied. The English What to Test notes and privacy URL are configured. External availability still requires Apple's beta review; the submission attempted on 2026-08-11 was rejected because required beta-review contact details, including the phone number, are missing. Apple does not send the usable external invitation until those details are supplied and the review is approved.
