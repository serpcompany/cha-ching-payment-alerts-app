...
TestFlight build `11` (`9b7476b8-ed3f-4005-bf31-3efe6f26df90`) is valid, `IN_BETA_TESTING`, and attached to the internal group `Cha-Ching Internal` (`483e4a13-43c0-4658-9c1b-8238e7b9a773`). It fixes the cold-launch crash when a user opens a payment notification from the Lock Screen, safely routes that press into the full payment details, and retains the genuine Apple notification surfaces, per-device switch, delayed lock-screen test, and streamlined Settings UI. The signed archive and Apple's processed build-icon endpoint both expose the dollar-symbol `ChaChingDollarIcon`. Builds `1` through `10` are superseded for internal testing.

The external group `Cha-Ching Beta` (`e95c8fca-3141-4ccb-917f-e7910bc37e0d`) contains `farleythecoder@gmail.com`, and build 8 remains attached. Build 11 is internal-only until the missing beta-review contact details are supplied. The English What to Test notes and privacy URL are configured. External availability still requires Apple's beta review; the submission attempted on 2026-08-11 was rejected because required beta-review contact details, including the phone number, are missing. Apple does not send the usable external invitation until those details are supplied and the review is approved.
...
Build `11` was signed manually with active provisioning profile `Cha-Ching App Store Push` because Xcode's cached interactive Apple Account session is expired. The profile contains production APNs and Sign in with Apple entitlements. The signed archive was uploaded with the App Store Connect API key; Apple processed it as `VALID` with non-exempt encryption set to false and made it available to the internal group.
