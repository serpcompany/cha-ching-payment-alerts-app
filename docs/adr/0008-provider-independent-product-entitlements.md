...
When an authenticated client receives **Subscription required**, it automatically reconciles a locally available verified current StoreKit entitlement before presenting the final access state. It does not force `AppStore.sync()` during launch or foreground refresh. Explicit Restore Purchases is the fallback that may ask Apple to synchronize account transactions and must report a restored, already-active, no-active-purchase, or error result.
