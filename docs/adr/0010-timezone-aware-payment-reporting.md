...
Expose a product-gated dashboard endpoint that aggregates every matching succeeded sale. The Worker builds UTC bucket boundaries from the saved timezone and submits count, currency, bucket, product, and source aggregate statements together through one D1 `batch()`, which D1 executes as a SQL transaction. Concurrent inserts, deletes, and updates therefore cannot mix different database moments within one dashboard response, and only response-sized grouped rows reach the Worker. The response returns a selected daily summary plus approved current/previous report windows, comparison states, zero-filled chart buckets, and product/source breakdowns. The daily summary defaults to Today and accepts a nonnegative day offset so the client can page backward without treating a future day as valid. Prior days span complete timezone-local calendar days; Today ends at generation time. Payment counts may span currencies, but monetary totals and averages remain separated by currency. The 100-row Payments endpoint remains independent.
