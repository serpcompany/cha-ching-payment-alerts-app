...
4. Migration `0016` assigns every sale a separate AUTOINCREMENT ingestion sequence and maintains it with an insert trigger. The Worker takes a stable maximum-sequence cutoff, reads matching succeeded rows with sequence-keyset pagination, and folds each page into report aggregates rather than retaining full history in Worker memory or using the 100-row Payments feed. SQLite does not reuse the deleted maximum AUTOINCREMENT value, so a concurrent backfill remains beyond the cutoff and waits for the next refresh. Monetary results remain separated by currency, and missing chart buckets are returned as zero.
