...
3. `GET /v1/dashboard` computes local calendar boundaries from the saved timezone while keeping every stored payment timestamp in UTC. Its daily summary accepts a nonnegative offset for backward/forward paging, caps forward movement at Today, and uses a complete local midnight-to-midnight window for prior days.
4. The Worker builds UTC bucket boundaries from the saved timezone, then executes selected daily-summary, current/previous totals, currency, bucket, product, and source aggregate statements together through one D1 `batch()`. D1 executes the batch as one SQL transaction, so concurrent inserts, deletes, and updates cannot produce a hybrid dashboard response. Only response-sized grouped rows reach the Worker; raw payment history does not. The selected daily summary retains the original `today` response key for compatibility with older clients. Monetary results remain separated by currency, and missing chart buckets are returned as zero.
